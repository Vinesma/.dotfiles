...
"$youtube_dl_path" --download-archive archive.txt \
    -x --audio-format "$audio_format" \
    --playlist-start "$start_at" --dateafter "$date" \
    -o "$output_string" "$playlist"

[[ "$?" -eq 0 ]] && "$notify_send_path" -i "$icon_info" "CRON" "[download-playlist] Playlist(s) have been downloaded" || \
    "$notify_send_path" -i "$icon_error" "CRON" "[download-playlist] Failed to download playlist(s)"
